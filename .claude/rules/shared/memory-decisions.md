...
## 2026-08-26: Ingresos agrupados por motivo — resuelto en lectura, con `Map` y ordenado por `createdAt`

El agrupamiento de la sección INGRESOS se resuelve en lectura, sin persistir un campo normalizado en `incomes` ni migrar datos: la clave es `reason.toLowerCase().trim()` + `currency`, así que dos monedas con el mismo motivo son dos líneas y ARS/USD nunca se suman. Se acumula con `Map` y no con `Record` porque un objeto de JS hoistea las claves *integer-like* al frente de `Object.values`, y un motivo numérico (ej. `"2024"`) saltaría al principio rompiendo el orden por primera aparición. Se ordena por `createdAt` antes de agrupar, no por el orden que devuelve Firestore: en meses retroactivos todos los ingresos comparten `date` (`buildBackdatedTimestamp`) y el desempate cae a doc ID, así que el motivo mostrado por grupo salía del documento equivocado. La función quedó privada y con su interfaz local a `report.service.ts` (mismo criterio que `ServiceWithInstallment` en `payment-method-report.service.ts`) por tener un único consumidor — es el único sitio del bot que lista ingresos. Los totales del header dual y del balance no se tocaron: se calculan antes del bloque de detalle sobre el array crudo, así que quedan idénticos por construcción y no por cuidado manual. Su nombre incluye la moneda (`groupIncomesByReasonAndCurrency`) porque la moneda-en-la-clave es el invariante que impide sumar ARS con USD, y un nombre lo protege mejor que un párrafo de JSDoc.

**Gotcha heredado, ampliado por la auditoría**: el bloque de gastos vecino agrupa con `Record` y elige su `displayName` según el orden de Firestore, así que arrastra **los dos** latentes que este ticket evitó — el de claves *integer-like* y el del desempate por doc ID en meses retroactivos (el mismo bug que acá se corrigió como T1). Diferido como ticket aparte.
