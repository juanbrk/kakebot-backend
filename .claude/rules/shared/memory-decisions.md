...
## 2026-07-22: Auditoría post-QA de la rama — logging de fallos de edición, fix de `editOrReply`, y corrección de drift de documentación
...
- **Scope**: los demás hallazgos de la auditoría quedan fuera de esta rama, como follow-ups (detalle completo también en `TICKET.md`, gitignoreado — se resume acá para sobrevivir al borrado del worktree):
  ...
  - **C — 4 call-sites write-then-`replyOrEdit`** vía helpers de render compartidos: `service.ts:531`/`:578` (`markInstallmentAsPaid`), `service.ts:940` (`updateServicePaymentMethod`), `tax.ts:637` (`updateTaxPaymentMethod`). Violan §9.4; inocuo hoy solo porque esas escrituras son idempotentes — deja de serlo si se cuelga una no idempotente detrás de `showInstallmentDetail`/`showServiceActionView`/`showTaxActionView`.
  - **D (menor) — Premisa no registrada**: varios sitios (ej. `card-stmt:129`) asumen que "hoy siempre se entra por callback". Si una ruta futura entra por texto, `replyOrEdit` manda mensaje nuevo en vez de editar — dos teclados activos, contra la decisión del 2026-07-03.
