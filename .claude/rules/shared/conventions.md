...
| `replyOrEdit(ctx, text, extra?)` | `helpers/telegram.ts` | Edit message when triggered from a callback (swallowing any edit error — a double-tap "not modified" is a silent no-op), else reply. Use for EVERY cosmetic edit (no preceding write): menu navigation, re-rendering screens, consuming a button. |
